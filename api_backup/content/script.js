...
router.get("/", async (req, res) => {
        res.send('connected the content.')
})
/* ---------------------------------------------------------------------------------- */
    ...
    storage: multer.diskStorage({
        destination: (req, file, cb) => {
            ...
            if (file.fieldname === 'file_sound') {
                uploadPath = path.join(__dirname, '../../domains/api-ecproject.poommer.in.th/public_html/media/sound/vocab', word_List);
            } else if (file.fieldname === 'file_image') {
                uploadPath = path.join(__dirname, '../../domains/api-ecproject.poommer.in.th/public_html/media/image/vocab', word_List);
            }
        ...
        filename: (req, file, cb) => {
            ...
            if (file.fieldname === 'file_sound') {
                filePath = path.join(__dirname, '../../domains/api-ecproject.poommer.in.th/public_html/media/sound/vocab', req.query.wl, word_en + path.extname(file.originalname));
            } else if (file.fieldname === 'file_image') {
                filePath = path.join(__dirname, '../../domains/api-ecproject.poommer.in.th/public_html/media/image/vocab', req.query.wl, word_en + '.webp');
            }
...
router.post("/vocab/", upload, async (req, res) => {
    try {
        const { word_en, word_th, pos, word_List, status_image} = req.body;
        ...
        let sql = "INSERT INTO vocab (vocab_ID, word_en, word_th, pos, word_cate, status_image) VALUES (?, ?, ?, ?, ?, ?)";
        conn.execute(sql, [genID, word_en, word_th, pos, word_List, status_image], (err, result) => {
            if (err) {
                res.status(500).json(err.message);
            }
            ...
            res.status(200).json({
                ...
                response:{
                    ...
                    word_List,
                    status_image,
                    imageURL: status_image === 1 ? `https://api-ecproject.poommer.in.th/media/image/vocab/${word_List}/${word_en}.webp` : 'none',
                    soundURL: `https://api-ecproject.poommer.in.th/media/sound/vocab/${word_List}/${word_en}.wav`
...
router.get("/vocab/", async (req, res) => {
    try{
        ...
        conn.execute("SELECT `vocab_ID`, `word_en`, `word_th`, `wl_name`, `pos_name`, `status_image` FROM `vocab` JOIN `wordlist` ON `vocab`.word_cate = `wordlist`.`wl_id` JOIN `partofspeech` ON `vocab`.pos = `partofspeech`.`pos_id` LIMIT ?,?; ",[startIndex, offset], (err, result) => {
            if(err){res.status(500).json({message:err.message})}
            const resultLest = result.map(item => {
                return{
                    ...item,
...
router.delete("/vocab/:vocabID", async (req, res) => {
    try {
        ...
        const oldSoundPath = path.join(__dirname, '../../domains/api-ecproject.poommer.in.th/public_html/media/sound/vocab', currentWordCate, currentWordEn + '.wav');
        const oldImagePath = path.join(__dirname, '../../domains/api-ecproject.poommer.in.th/public_html/media/image/vocab', currentWordCate, currentWordEn + '.webp');
