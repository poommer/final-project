...
//   status_send = false
  ...
  let clickToSpeck = () => {
    ...
    if(micStatus){
        ...
        recognition.addEventListener('result', (event) => {
            ...
            if (result === word.word_en.replace(/[^a-zA-Z0-9\s]/g, '').toLowerCase()) {
                console.log('Correct!');
                ansCheck = true;
                recognition.stop();
  ...
  <div class="w-full h-full flex flex-col md:flex-row gap-8">
    <div class="h-[400px] w-[320px] flex flex-col justify-center items-center bg-white p-2 border-4 border-rose-700 rounded-3xl overflow-hidden">
    ...
    <div class="flex flex-col gap-6 justify-start items-center">
        ...
        <div class="w-full md:w-[25rem] rounded-xl text-5xl py-[14px] px-2 flex justify-center items-center bg-white shadow-[10px_10px_0px_0px_#373C4A] border-2 border-ec-dark-blue-1">
            {word.word_en}
        </div>
        {#if msg !== ''}
            <p class={`bg-zinc-300 p-2 w-full flex justify-center items-center rounded-lg text-zinc-700`}>{msg}</p>
        {/if}
        {#if ansCheck === null || (ansCheck === false && maxAns > 0) }    
          <button class={`w-[4rem] h-[4rem] ${micStatus === true ? 'bg-green-300 animate-pulse animate-infinite' : 'bg-ec-purple-600'} rounded-full flex justify-center items-center`} on:click={clickToSpeck}>
