<script>
  ...
  import { createEventDispatcher } from 'svelte';
   ...
   const dispatch = createEventDispatcher();
    ...
    const selectedVocab = vocab.find(val => value === val.vocab_ID);

    if (!selectedVocab) return; // ป้องกันกรณี vocab_ID ไม่ตรง

    const wordEng = selectedVocab.word_en;
    const wordTH = selectedVocab.word_th;

    if (event.target.checked) {
        vocab_checked.push(value);
        vocab_checkedShow.push({ wordEng, wordTH });
    } else {
        // ค้นหาตำแหน่งของค่าที่ต้องการลบ
        let vo = vocab_checked.indexOf(value);
        let word_index = vocab_checkedShow.findIndex(obj => obj.wordEng === wordEng);
    ...
    // Dispatch event พร้อมข้อมูลใหม่
