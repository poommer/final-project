...
//   import CreateConversation from '../../../../../../lib/admin/createConversation.svelte';
  ...
  const send_data = async () => {
      ...
      try{
          ...
          const letest_ID = await respone_lesson.data.row_num + 1
       ...
       const respone = await Promise.all([ ...Vocab_create, ...create_sentenec])
        
        console.log(respone);
        window.location = `/admin/lesson/topic/detail?id=${topic_ID}`;
    } catch (err) {
        open_popup = true
...
<div class="flex justify-end mt-2 gap-4">
        ...
        ${cur_stap <= 1 || cur_stap === 5 ? 'hidden' : '' }`}
        
        >
        {cur_stap === 4 ? 'ยกเลิก' : 'ย้อนกลับ'}
    </button>
        
        <button 
        on:click={()=>{cur_stap === 4 ? send_data() : next_to()}} 
        class={` 
        w-[8rem] rounded  text-white text-2xl p-2 flex justify-center items-center transition-all duration-[0.5s] ease-in-out
        ${cur_stap === 4 ? 'bg-green-600 hover:bg-green-500' : 'hover:bg-ec-purple-800 bg-ec-purple' } 
        ${cur_stap === 5 ? 'hidden' : '' }`}>
        {cur_stap === 4 ? 'ยืนยัน' : 'ถัดไป'}
    </button>
    </div>
</div>
