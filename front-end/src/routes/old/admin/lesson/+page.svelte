...
const Delete_lesson = async (ID, type) => {
    ...
    if(response.data.status === 200){
        const index_lesson = ID.split('-')[0] - 1;
        const index_topic = ID.split('-')[1] - 1;
        type === 'topic' ? lesson[index_lesson].lesson.splice(index_topic, 1) : lesson.splice(index_lesson, 1)
        lesson = [...lesson]
    }else{
        ...
        <div class="bg-gray-200 p-4 flex flex-col justify-between rounded-xl">
            <div class="flex justify-between items-center">
                ...
                <div class="flex justify-end items-end gap-2">
                    <button on:click={() => {goto(`/admin/lesson/topic/${Item.heading.id.split('-')[0]}/create`)}}>
                    ...
                    <button 
                    on:click={()=>{Delete_lesson(Item.heading.id)}}>
            ...
            <div class="m-4  p-4">
                ...
                <div class="flex items-center ">
                        ...
                        <a href={`./lesson/topic/${les.lesson_ID}/detail`} class="text-gray-600 text-2xl underline ml-2 mr-2">{les.lesson_title}</a>
                        <button>
